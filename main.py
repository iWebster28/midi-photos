...
def main():
    ...
    x_offset_slider_middle = size[0] / 2
    const_scale = size[0] / 16383
        ...
        for message in port:
            ...
            if hasattr(message, 'control') and message.control == 60: # Jog wheel
                    if message.value == 1:
                        # fine grain adjust
                        delta = 1
                        pitch_delta = 128 * delta if not buffer[2] >= 8191 else 0
                        last_sampled_pitch += pitch_delta
                        buffer[2] += pitch_delta
                        if pitch_delta != 0:
                            pyautogui.dragRel(1, 0, button='left')
                        print(f'fine grain CW {buffer[2]}')
                    elif message.value == 65:
                        # fine grain adjust
                        delta = -1
                        pitch_delta = 128 * delta if not buffer[2] <= -8192 else 0
                        last_sampled_pitch += pitch_delta
                        buffer[2] += pitch_delta
                        if pitch_delta != 0:
                            pyautogui.dragRel(-1, 0, button='left')
                        print(f'fine grain CCW {buffer[2]}')

            elif len(buffer) == 3: # Slider 
                ...
                if count == 0:
                    pyautogui.moveTo(coords[0] + (const_scale * message.pitch) + x_offset_slider_middle, coords[1] + 1)

                elif count % 16 == 0 or buffer[2] == 8191 or buffer[2] == -8192:
                    pyautogui.dragTo(coords[0] + (const_scale * buffer[2]) + x_offset_slider_middle, coords[1] + 1, button='left')
